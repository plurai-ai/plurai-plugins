...
# with the latest src/. Python doesn't hot-reload modules, so a server
# that started before your code change keeps the old code in memory
# forever — running this is the cleanest reset.
#
# Usage:
#   ./dev-restart.sh          # kill all running server.py processes
#
# Then in Claude Code: /mcp → pluto-judge → Reconnect.

set -e

PIDS=$(pgrep -f "pluto-judge.*src/server\.py" || true)
...
sleep 2
STILL=$(pgrep -f "pluto-judge.*src/server\.py" || true)
if [ -n "$STILL" ]; then
