...
If the user provides a labeled data file, after `start_evaluator` call `upload_data` with the parsed records and `example_set_id`. Continue with `ask_user` using the refinement questions from the start_evaluator response.
