...
Call `search_evaluators` first as an optimization to check whether the user already has an evaluator in their Plurai workspace that fits this task. **If the list is empty, say nothing about it and proceed silently to create a new one** — a fresh user has no evaluators yet and should not be told something is missing. If one or more existing evaluators match, surface the full list to the user and use `ask_user` to ask whether to reuse one or create a new one. If reusing, jump straight to step 6 (integration snippet): the endpoint URL is the `endpoint_url` from the search result for the chosen evaluator, and the API key comes from `get_api_key` (the same Plurai API key the user configured at session start — no new key is created).
...
**Data path — if the user supplied a labeled data file** (e.g. `/eval --data path/to/file.csv` or they pasted a path): after `start_evaluator`, read the file, parse it into `{sample, label, reasoning?}` records, then call `upload_data` with the `example_set_id` from the start_evaluator response. Do NOT synthesize records. Continue with `ask_user` using the refinement questions from the start_evaluator `agent_response`.
...
6. **When results land:** show baseline vs trained metrics (accuracy, precision, recall) and the improvement delta. Frame the trained row as "vibe-trained" if the user picked SLM and as "optimized" if they picked LLM. Then settle the integration language by repo inference — `Glob` the working directory root only (no recursion; depth-mixed manifests are the polyglot case):
   ...
   **On unambiguous inference: emit the snippet directly — no `ask_user`, no picker, no Recommended marker.** Prefix it with one plain-text line: `Detected a Python project — emitting Python. Reply with JavaScript/TypeScript or cURL if you'd like a different language.` (substitute the detected language). On ambiguous / missing signal only, call `ask_user` with header `"Language"`, question `"Which language should I emit the integration snippet in?"`, options Python, JavaScript/TypeScript, cURL.
   ...
   Once the language is settled, call `get_api_key` to retrieve the user's existing Plurai API key (configured at session start — local read, **not** a new key creation) and embed it in the integration snippet. The integration code MUST format the input using the same template specified in the task description. For example, if the evaluator uses "## Context:\n{context}\n\n## Response:\n{response}", the code must combine the fields into a single string following that exact template before sending to the endpoint. The evaluator only accepts a single message — never multiple messages. The language ask (when shown) is the only post-results user question — do NOT ask whether to create a key or whether to integrate.
